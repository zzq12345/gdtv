...
必须通过 Node.js + WASM 签名才能获取播放地址。

本脚本不再提供备选公开源，若签名失败则直接报错。
═══════════════════════════════════════════════════════════
...
用法示例：
  ...
  python gdtv_proxy.py --proxy --source "http://your-custom-source/live.m3u8"  # 可手动指定源
"""
...
# WASM 签名相关（必须，需要 Node.js 环境 + lizhi.wasm）
...
def get_play_url(force=False, custom_source=None):
    ...
    if custom_source:
        ...
        # 2) 必须通过 Node.js + WASM 签名
        try:
            url = fetch_via_nodejs()
            print(f"[2] 通过 API 签名获取: {url}")
        except Exception as e:
            # 不再降级，直接抛出异常
            raise RuntimeError(f"签名获取失败，无备选源: {str(e)}")
